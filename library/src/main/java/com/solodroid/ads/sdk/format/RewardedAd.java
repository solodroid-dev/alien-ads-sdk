        ...
        public void loadRewardedAd(OnRewardedAdCompleteListener onComplete, OnRewardedAdDismissedListener onDismiss) {
            if (adStatus.equals(AD_STATUS_ON) && placementStatus != 0) {
                switch (mainAds) {
                    ...
                    case ALIEN:
                    case ALIEN_ADS:
                        PropsAdsManagement.loadRewardedAds(activity, alienAdsRewardedId, new RewardedAdLoadCallback() {
                            @Override
                ...
                switch (backupAds) {
                    ...
                    case FAN_BIDDING_AD_MANAGER:
                        com.google.android.gms.ads.rewarded.RewardedAd.load(activity, adManagerRewardedId, Tools.getGoogleAdManagerRequest(), new RewardedAdLoadCallback() {
                            ...
                            public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                                Log.d(TAG, loadAdError.toString());
                                adManagerRewardedAd = null;
                                Log.d(TAG, "[" + backupAds + "] " + "failed to load rewarded ad: " + loadAdError.getMessage() + ", try to load backup ad: " + backupAds);
                            }
                        });
                        break;
                    ...
                    case ALIEN_ADS:
                        PropsAdsManagement.loadRewardedAds(activity, alienAdsRewardedId, new RewardedAdLoadCallback() {
                            ...
                            @Override
                            public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                                Log.d(TAG, "[" + mainAds + "] " + "failed to load rewarded ad: " + loadAdError.getMessage());
                            }
                        });
                        break;
                ...
                switch (mainAds) {
                    ...
                    case ALIEN_ADS:
                        if (PropsAdsManagement.getRewardedAds() != null) {
                            PropsAdsManagement.triggerRewardedAds(activity, rewardItem -> {
                                onComplete.onRewardedAdComplete();
                                Log.d(TAG, "The user earned the reward.");
                            });
                            new Handler(Looper.getMainLooper()).postDelayed(()-> loadRewardedAd(onComplete, onDismiss), 1000);
                        } else {
                            showRewardedBackupAd(onComplete, onDismiss, onError);
                        }
                        break;
